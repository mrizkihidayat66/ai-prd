...
- Unquoted multi-word ERD relationship labels`,
  ...
  promptContext: {
    title: 'AI Bootstrap Context',
    instruction: `Write a comprehensive "AI Bootstrap Context" document that an AI coding agent will read at the start of every session to orient itself. This is the most important onboarding document for the project.

Structure it with the following sections using markdown headings:

## Project Identity
- Project name and one-sentence purpose
- The core problem being solved and for whom
- Key success metrics or definition of done

## Technical Stack
- All major technologies with versions (e.g., "Next.js 15 (App Router), TypeScript 5, PostgreSQL 16 via Prisma")
- Key libraries and their specific role (not just a list)
- Infrastructure and deployment target

## Architecture Overview
- System design philosophy (e.g., monolith, microservices, server-first, etc.)
- Key architectural patterns used (Repository, CQRS, domain-driven, etc.)
- How data flows through the system at a high level
- Critical boundaries and constraints between layers

## Project Structure
- Key directories and what they contain
- Where to find: API handlers, data models, UI components, utilities, tests
- Any unusual conventions in file naming or organization

## Developer Quick Start
- Commands to start the dev server, run tests, run linter, apply migrations
- How to set up environment variables or local config

## Working With This Codebase
- The pattern to follow when adding a new API endpoint
- The pattern to follow when adding a new UI component
- The pattern to follow when adding a new database model or migration
- How to run and structure tests

## Non-Negotiable Constraints
- 3–5 architectural decisions that MUST be respected and WHY
- Common mistakes to avoid in this codebase
- Anything that should never be changed without explicit approval

Keep the document focused and accurate. Use code snippets for commands and paths. Aim for 500–800 words. This document will be read by AI agents on every session start.`,
  },
  effortEstimate: {
